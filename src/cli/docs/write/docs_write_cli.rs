...
impl DocsWriteArgs {
    ...
    pub async fn invoke(self) -> Result<()> {
        ...
        let (docs_dir, file_count) = tokio::task::spawn_blocking(move || -> Result<(PathBuf, usize)> {
            let snapshots = generate_help_snapshots()?;
            let docs_dir = output_path.join("command-help");
            std::fs::create_dir_all(&docs_dir)
                .wrap_err_with(|| format!("Failed to create docs directory {}", docs_dir.display()))?;

            for snapshot in &snapshots {
                let file_path = docs_dir.join(&snapshot.file_name);
                let content = format!("$ {}\n\n{}", snapshot.invocation, snapshot.output.trim_end());
